...
```json
{
  ...
  "bazi": {
    ...
    "流年": { "year", "pillar", "干十神" },
    "真太阳时修正": {  // v0.5 新增：修正详情供审计（仅 --true-solar-time 时填充）
      "原始钟表时间": "1946-06-14 10:54",
      "时区": "America/New_York",
      "UTC_offset_hours": -4.0,         // pytz 自动识别（含夏令时）
      "时区中央经线": -60.0,
      "出生地经度": -73.7917,
      "经度修正_分钟": -55.17,
      "时差方程_分钟": 0.01,             // v0.5 新增（NOAA 近似公式）
      "合计修正_分钟": -55.15,
      "真太阳时时刻": "1946-06-14 09:58:50"
    }
  },
  "astrology": {
...
`--true-solar-time` 开启后，paipan 自动做 **3 层修正**（互相独立但合并应用）：

| 层 | 修正内容 | 范围 | 实现 |
|---|---|---|---|
| 1 | **经度修正** | (出生地经度 − 时区中央经线) × 4 分钟 | 已有 |
| 2 | **夏令时自动识别** | pytz 历史 DST 规则 | 已有 |
| 3 | **时差方程**（Equation of Time）| ±0-16 分钟 / 取决于月份 | **v0.5 新增** ⭐ |

**时差方程为什么重要**：地球轨道椭圆 + 地轴倾斜，引起真太阳时与平均太阳时（钟表时间）每日有 ±0-16 分钟天文偏差。
...
→ **2 月 / 11 月** 出生的人 + **时辰边界 ±20 分钟** 出生时间 = 时差方程**改变时柱判定**。
...
- 1996-02-15 13:08 上海
  - 未修正：时柱"丁未"（13:08 在未时）
  - 修正后：时差方程 -14.57 分钟 + 经度修正 +5.89 分钟 = -8.68 分钟 → 真太阳时 12:59:19 → 时柱"**丙午**"
  - 时柱跨时辰 ✓ 时差方程在边界案例真实改变判定

**审计输出**：每次开启 `--true-solar-time`，JSON 输出 `真太阳时修正` 字段，含 9 项可审计数据（原始时间 / 时区 / 经度修正 / 时差方程 / 合计 / 真太阳时时刻 等）。咨询师可向客户展示作为专业证明。
