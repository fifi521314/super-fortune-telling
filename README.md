...
- **R44-R45 v2.0**：**方法论层**（R44 突出矛盾扫描 + R45 v2.0 古典格局优先 / 入口）
- **R46-R52**：**地支干天互动 — 合冲刑害完整体系**（R46 六合 + R47 三合三会半合拱合 + R48 三刑自刑 + R49 六害六破 + R50 天干五合 + R51 动静博弈复合规则 + R52 子平vs盲派 sync 框架）
- **R53-R56**：**格局识别层**（R53 十大正格 + R54 化气/专旺/从格 + R55 复合格局 + R56 完整 SOP）
- **R57**：**神煞精选层**——桃花专项规则（四桃花 + 局桃花并用 / 与十神叠加 / 桃花互动 / 大运流年触发 / 严格脱毒边界）
- **R58-R60**：**议会自检 + 实操层**（防错机制——R58 v2.0 十神标注自检 + 锚点表先于推理 + R59 现实变量必问 + R60 城市方位双轨实地法 —— 全部来源命主反馈触发的真实 agent 错误修复，living-doc 哲学体现）
- **R61-R63**：**现代心理叙事层（古今并陈 + 用户隐私保护）**——R61 十神缺位的现代脱毒读法 + R62 v2.0 古今并陈叙事框架（参考 Tompkins 心理占星范式，强制三段：【古意】+【现代心理解读】+【具体场景】）+ **R63 用户反馈匿名化原则**（隐私保护强制规则——所有用户反馈触发的 skill 升级在公开文件中必须匿名）
- **R64**：**资源来源判定（自致 vs 他致）**——防 agent 把"印重/官杀旺"美化成"个人能力"，主动区分"自己挣的"vs"他人给的（出身/婚姻/平台/庇护）"；瘦身后第一条走完整准入门槛升级的规则（3 盘泛化验证 + 防错性质）
- **R52 v2.0 + R55 v2.0 升级**（2026-05-27）：盲派做功路径**必扫描**（不仅在分歧时用）+ 复合格局增至 7 个含**比劫库冲财库**（盲派经典暴利做功格局）—— 1991 北京男案例触发的范式级升级

**配套**：`references/bazi-patterns.md` —— **33 格局完整手册**（10 正格 + 5 化气 + 5 专旺 + 6 从格 + **7 复合**（新增比劫库冲财库）+ 魁罡指针 + 杂格附录）含成格 / 破格 / 临界 / 喜忌 / 现代脱毒意象 / 常见误判 / 工作流 SOP / 破格条件总论。
...
├── LICENSE                   # MIT
├── references/
│   ├── rules-engine.md       # 64 条核心规则（含 R44/R45 方法论 + R46-R52 合冲刑害 + R53-R56 格局识别 + R57 桃花 + R58-R60 防错 + R61-R63 古今并陈/隐私 + R64 资源来源判定）
│   ├── bazi-patterns.md      # 33 格局完整手册（10 正格 + 5 化气 + 5 专旺 + 6 从格 + 7 复合 + 魁罡指针 + 杂格附录 + SOP）
